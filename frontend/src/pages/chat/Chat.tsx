...
const Chat = () => {
    ...
    const onShowIntents = (intents: string[]) => {
        setIntents(intents)
    ...
    const onViewSource = (citation: Citation) => {
        if (citation.url && !citation.url.includes('blob.core')) {
            const url = 'https://stmyconnect.blob.core.windows.net/marketing/blobs/' + citation.url + '?sp=r&st=2024-07-30T21:44:21Z&se=2124-07-31T05:44:21Z&spr=https&sv=2022-11-02&sr=c&sig=mKcyQtb%2FyTRYdLc%2BIOkwoxqMEpsrZCeXrynOkZQkpJ8%3D'
    ...
    const parseCitationsFromMessage = (message: ChatMessage) => {
        if (message?.role && message?.role === 'tool') {
            try {
                return JSON.parse(message.content).citations;
            } catch {
                return []
            }
        }
    ...
    const parseIntentsFromMessage = (message: ChatMessage) => {
        if (message?.role && message?.role === 'tool') {
            try {
                return JSON.parse(JSON.parse(message.content).intent)
            } catch {
                return []
            }
        }
                                            ...
                                            <div className={styles.chatMessageGpt}>
                                                <Answer
                                                    answer={{
                                                        ...
                                                        citations: parseCitationsFromMessage(messages[index - 1]),
                                                        intents: parseIntentsFromMessage(messages[index - 1]),
                                                        message_id: answer.id,
                                                        feedback: answer.feedback,
                                                    }}
                                                    onCitationClicked={c => onShowCitation(c)}
                                                    onIntentsClicked={(c) => onShowIntents(c)}
                                                />
                                            </div>
                                        ) : answer.role === ERROR ? (
                            ...
                            <h5
                                ...
                                title={
                                    activeCitation.url && !activeCitation.url.includes('blob.core')
                                        ? activeCitation.url.substring(activeCitation.url.lastIndexOf('/') + 1)
